...
| Column                        | Description                                                                                                                                                                              |
|-------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **contig \| bin**             | The identifier of the contig or bin in which the motif was discovered. Contigs are individual assembled sequences, while bins are groups of related contigs that together approximate a draft genome. |
...
| **motif**                     | The detected DNA motif sequence. This may use [IUPAC degenerate nucleotide codes](https://www.bioinformatics.org/sms/iupac.html) to represent positions where multiple nucleotides are possible. |
...
| **motif_complement**          | The reverse complement of the motif, representing the motif on the opposite DNA strand.                                                                                                 |
| **mod_position_complement**   | The zero-based position of the modified base in the motif complement.                                                                                                                    |
| **n_mod_complement**          | The number of generally methylated occurrences found on the complementary strand.                                                                                                         |
| **n_nomod_complement**        | The number of generally unmethylated occurrences found on the complementary strand.                                                                                                       |
...
**Overview:**  
The bin improvement module refines genome bins by identifying contigs that may be contaminating them. This involves checking whether contigs consistently cluster differently from the rest of their bin across multiple clustering methods. Suspect contigs are recorded in `bin_contamination.tsv`. If requested, cleaned bins (with contaminants removed) can be created.
...
Contigs appear in this file if all four clustering methods (agg, spectral, gmm, hdbscan) assign the contig to a cluster different from the bin’s main cluster, suggesting contamination.
...
Assigning unbinned contigs to bins uses classification methods (Random Forest, Linear Discriminant Analysis, and k-Nearest Neighbors) trained on methylation data. The results are recorded here.
